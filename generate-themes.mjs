#!/usr/bin/env node
/**
 * LiveCams — Theme Generator
 * Produces one HTML file per color in themes/<slug>.html
 */
...
const COLORS = [
  // Exact hex provided
  { name: 'Oxidized Espresso',  hex: '#2B160C' },
  { name: 'Blackened Ivy',      hex: '#0F2A22' },
  { name: 'Shadowed Teal',      hex: '#153638' },
  { name: 'Ancient Moss',       hex: '#3F5A46' },
  { name: 'Sage Smoke',         hex: '#6F7F6C' },
  { name: 'Burnt Rust',         hex: '#A13E1E' },
  { name: 'Velvet Burgundy',    hex: '#831B1A' },
  { name: 'Muted Ochre',        hex: '#AE8452' },
  { name: 'Tarnished Gold',     hex: '#C2A46B' },
  // Named — curated hex values
  { name: 'Ink Black',          hex: '#0E0E0E' },
  { name: 'Aged Bronze',        hex: '#8B6A14' },
  { name: 'Walnut Leather',     hex: '#5C3318' },
  { name: 'Dusty Oat',          hex: '#C1B39A' },
  { name: 'Burnt Umber',        hex: '#8A3324' },
  { name: 'Twilight Blue',      hex: '#2C3E6B' },
  { name: 'Olive Smoke',        hex: '#5B6040' },
  { name: 'Dusty Plum',         hex: '#6B4E6E' },
  { name: 'Ash Grey',           hex: '#8C8C8C' },
  { name: 'Ombre',              hex: '#4A3A6B' },
  { name: 'Espresso',           hex: '#3D1C0C' },
  { name: 'Rum',                hex: '#8B4514' },
  { name: 'Old Wood',           hex: '#4A3728' },
  { name: 'Dusty Beige',        hex: '#C4AA8A' },
  { name: 'Dearth',             hex: '#6B5A4E' },
  { name: 'Antique Ivory',      hex: '#E6DCCA' },
  { name: 'Deep Chestnut',      hex: '#6B2F2F' },
  { name: 'Forest Moss',        hex: '#3D5C3A' },
  { name: 'Oxford Brown',       hex: '#3D2B1F' },
  { name: 'Charcoal Slate',     hex: '#3D4A52' },
  { name: 'Vintage Maroon',     hex: '#6B1F30' },
  { name: 'Muted Olive',        hex: '#7A8A5A' },
  { name: 'Faded Gold',         hex: '#C4A84A' },
  { name: 'Dusty Taupe',        hex: '#8C7B6B' },
  { name: 'Black Chocolate',    hex: '#1A0F0A' },
  { name: 'Caput Mortuum',      hex: '#592720' },
  { name: 'Cinereous',          hex: '#8C7B70' },
  { name: 'Silk Grey',          hex: '#A8A2A0' },
  { name: 'Moody Elegance',     hex: '#2A2035' },
  { name: 'Modern Luxe',        hex: '#1A1A2E' },
  { name: 'Earthy Dark Tones',  hex: '#3D2E20' },
  { name: 'Cool & Calming',     hex: '#1E2E3D' },
  { name: 'Warm & Cozy',        hex: '#3D2010' },
  { name: 'Pickle',             hex: '#5C7A3A' },
  { name: 'Wooster Olive',      hex: '#5C6140' },
  { name: 'Raw Plaster',        hex: '#C4B89A' },
  { name: 'Oatmeal',            hex: '#C0AA8A' },
  { name: 'Mole',               hex: '#6B5C52' },
  { name: 'Bedford Brown',      hex: '#4A3020' },
  { name: 'Smoke',              hex: '#6B6B6B' },
  { name: 'Cinder',             hex: '#2A2A2A' },
  { name: 'Bleecker',           hex: '#4A6B8A' },
  { name: 'Damson',             hex: '#5C1F5C' },
  { name: 'Mulberry',           hex: '#7A2B5A' },
  { name: 'Crosby',             hex: '#8A4A1A' },
  { name: 'Hudson Black',       hex: '#0A0A14' },
  { name: 'Madison Grey',       hex: '#4A5262' },
  { name: 'Thunder',            hex: '#2A2A3A' },
  { name: 'Rivington',          hex: '#2A3A4A' },
  { name: 'Mott',               hex: '#6B3A20' },
  { name: 'Bowery Blue',        hex: '#1A3A6B' },
  { name: 'Mercer Green',       hex: '#1A4A2A' },
  { name: 'Ruby Bear',          hex: '#8A1A2A' },
  { name: 'Black Raspberry',    hex: '#3A0A2A' },
  { name: 'Forestry',           hex: '#1A3A1A' },
  { name: 'Moss Gardens',       hex: '#3A5230' },
];
 ...
 * The hue anchors everything; saturation is gently reduced on backgrounds.
 */
function derivePalette(hex) {
  const rgb = hexToRgb(hex);
  const { h, s } = rgbToHsl(rgb);
  const sat = clamp(s * 0.55, 3, 28); // subtle tint, never grey or oversaturated

  // Accent: use original color if bright enough, otherwise boost lightness
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #search:focus { border-color: var(--border-hover); background: color-mix(in srgb, var(--bg-input) 80%, var(--accent) 5%); }
    ...
    .status-bar { display: flex; align-items: center; justify-content: space-between; margin-bottom: 22px; min-height: 24px; }
    ...
    /* ── CARD ───────────────────────────────────────── */
    .card {
      break-inside: avoid; display: block; margin-bottom: 14px;
    ...
    .card-thumb {
      ...
      background: color-mix(in srgb, var(--bg-card) 70%, var(--bg));
      overflow: hidden;
    }
    ...
    .card-thumb-placeholder { width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; color: var(--text-muted); }
    ...
    #state-loading, #state-empty, #state-error {
      ...
      color: var(--text-muted); gap: 14px; text-align: center;
    }
    #state-loading.visible, #state-empty.visible, #state-error.visible { display: flex; }
...
<header>
  <a class="logo" href="#">
    ...
    LiveCams
    <span class="logo-theme">${colorName}</span>
  </a>
...
<main>
  <div class="status-bar">
    <div class="status-count" id="status-count" aria-live="polite"></div>
  </div>
...
<script>
  ...
  let allRooms = [], filtered = [], currentPage = 1, searchQuery = '';
  ...
  async function init() {
    ...
    try {
      ...
      allRooms   = Array.isArray(data.results) ? data.results : [];
      if (!allRooms.length) throw new Error('No rooms returned.');
      $meta.textContent = allRooms.length.toLocaleString() + ' online';
      applyFilter();
      showState('grid');
    } catch (err) {
      $errMsg.textContent = 'Failed to load rooms: ' + err.message;
      showState('error');
    }
  }
  ...
  function render() {
    $grid.innerHTML = '';
    if (!filtered.length) { showState('empty'); updatePagination(0); return; }
    showState('grid');
    const totalPages = Math.ceil(filtered.length / PER_PAGE);
    currentPage = Math.min(currentPage, totalPages);
    const start = (currentPage - 1) * PER_PAGE;
    const slice = filtered.slice(start, start + PER_PAGE);
    const from  = start + 1, to = Math.min(start + PER_PAGE, filtered.length);
    $count.innerHTML = (filtered.length < allRooms.length
      ? '<strong>' + filtered.length.toLocaleString() + '</strong> results'
      : '<strong>' + filtered.length.toLocaleString() + '</strong> rooms live')
      + ' — showing ' + from + '–' + to;
    const frag = document.createDocumentFragment();
    slice.forEach(r => frag.appendChild(buildCard(r)));
  ...
  function buildCard(room) {
    ...
    if (imgSrc) {
      ...
      img.src = imgSrc; img.alt = username; img.loading = 'lazy'; img.decoding = 'async';
    ...
    const bl = document.createElement('span'); bl.className = 'badge-live'; bl.textContent = 'Live';
    const bv = document.createElement('span'); bv.className = 'badge-viewers';
    bv.innerHTML = '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>';
    bv.appendChild(document.createTextNode(' ' + viewers));
    thumb.appendChild(bl); thumb.appendChild(bv);
    a.appendChild(thumb);

    const body = document.createElement('div'); body.className = 'card-body';
    const nameEl = document.createElement('div'); nameEl.className = 'card-name'; nameEl.textContent = username;
    body.appendChild(nameEl);
    if (tags.length) {
      const tw = document.createElement('div'); tw.className = 'card-tags';
      tags.forEach(t => { const sp = document.createElement('span'); sp.className = 'tag'; sp.textContent = t; tw.appendChild(sp); });
      body.appendChild(tw);
    }
    a.appendChild(body);
    return a;
